function outputObserving(derivation) {
  return `(observing: [${derivation.observing
    .map(o => {
      return `{${o.name}:${o.lowestObserverState}}`;
    })
    .join('')}])`;
}

function consoleDDS(derivation, str) {
  console.log(
    `${str} ${derivation.runId} dds:`,
    derivation.dependenciesState,
    outputObserving(derivation)
  );
}

function outputDerivation(obs) {
  return `(observers: [${obs.observers
    .map(o => {
      return `{runID(${o.runId}):${o.dependenciesState}}`;
    })
    .join('')}])`;
}
function consoleLowest(obs, str) {
  console.log(
    `${str} ${obs.runId} lowest:`,
    obs.lowestObserverState,
    outputDerivation(obs)
  );
}
          ...
          (function(global) {
            ...
            function reaction(expression, effect, arg3) {
              ...
              opts.fireImmediately =
                arg3 === true || opts.fireImmediately === true;
            ...
            var IObservableFactories = (function() {
              ...
              IObservableFactories.prototype.ref = function() {
                if (arguments.length < 2) {
                  return createModifierDescriptor(
                    referenceEnhancer,
                    arguments[0]
                  );
                } else {
            ...
            var ComputedValue = (function() {
              ...
              ComputedValue.prototype.computeValue = function(track) {
                ...
                var res;
                if (track) {
                  res = mockup.trackDerivedFunction(
                    this,
                    this.derivation,
                    this.scope
                  );
            ...
            (function(IDerivationState) {
              IDerivationState[(IDerivationState['NOT_TRACKING'] = -1)] =
                'NOT_TRACKING';
              IDerivationState[(IDerivationState['UP_TO_DATE'] = 0)] =
                'UP_TO_DATE';
            ...
            mockup.shouldCompute = function(derivation) {
              consoleDDS(derivation, '[shouldCompute]');
                ...
                case IDerivationState.POSSIBLY_STALE: {
                  ...
                  for (var i = 0; i < l; i++) {
                    ...
                    if (isComputedValue(obj)) {
                      ...
                      if (
                        derivation.dependenciesState === IDerivationState.STALE
                      ) {
                        untrackedEnd(prevUntracked);
                        return true;
                      }
            ...
            mockup.trackDerivedFunction = function(derivation, f, context) {
              consoleDDS(derivation, '[trackDerivedFunction][before]');
              changeDependenciesStateTo0(derivation);
              ...
              mockup.bindDependencies(derivation);
              consoleDDS(derivation, '[trackDerivedFunction][after]');
              return result;
            };
            mockup.bindDependencies = function(derivation) {
              consoleDDS(derivation, '[bindDependencies][before]');
              ...
              consoleDDS(derivation, '[bindDependencies][after]');
            };
            function clearObserving(derivation) {
              ...
              consoleDDS(derivation, '[clearObserving][before]');

              derivation.dependenciesState = IDerivationState.NOT_TRACKING;

              consoleDDS(derivation, '[clearObserving][after]');
            ...
            function changeDependenciesStateTo0(derivation) {
              consoleDDS(derivation, '[changeDependenciesStateTo0][before]');

              if (derivation.dependenciesState === IDerivationState.UP_TO_DATE)
                return;
              derivation.dependenciesState = IDerivationState.UP_TO_DATE;

              consoleDDS(derivation, '[changeDependenciesStateTo0][after]');
              ...
              while (i--) {
                consoleLowest(
                  obs[i],
                  `[changeDependenciesStateTo0][before][${i}]`
                );

                obs[i].lowestObserverState = IDerivationState.UP_TO_DATE;
                consoleLowest(
                  obs[i],
                  `[changeDependenciesStateTo0][after][${i}]`
                );
              }
            }
            var persistentKeys = [
            ...
            function invariantObservers(observable) {
              ...
              invariant(
                list.length === 0 ||
                  Object.keys(map).length === list.length - 1,
            ...
            function addObserver(observable, node) {
              ...
              consoleDDS(node, '[addObserver][before]');

              consoleLowest(observable, `[addObserver][before]`);

              if (observable.lowestObserverState > node.dependenciesState)
                observable.lowestObserverState = node.dependenciesState;

              consoleDDS(node, '[addObserver][after]');
              consoleLowest(observable, `[addObserver][after]`);
            }
            ...
            mockup.reportObserved = function(observable) {
            ...
            function propagateChanged(observable) {
              ...
              consoleLowest(observable, `[propagateChanged][before]`);

              observable.lowestObserverState = IDerivationState.STALE;
              consoleLowest(observable, `[propagateChanged][after]`);
              ...
              while (i--) {
                ...
                consoleDDS(d, `[propagateChanged][before][${i}]`);

                if (d.dependenciesState === IDerivationState.UP_TO_DATE)
                  d.onBecomeStale();
                d.dependenciesState = IDerivationState.STALE;

                consoleDDS(d, `[propagateChanged][after][${i}]`);
              }
            }
            function propagateChangeConfirmed(observable) {
              consoleLowest(observable, `[propagateChangeConfirmed][before]`);

              if (observable.lowestObserverState === IDerivationState.STALE)
                return;
              observable.lowestObserverState = IDerivationState.STALE;
              consoleLowest(observable, `[propagateChangeConfirmed][after]`);
              ...
              while (i--) {
                ...
                consoleDDS(d, `[propagateChangeConfirmed][before][${i}]`);
                if (d.dependenciesState === IDerivationState.POSSIBLY_STALE)
                  d.dependenciesState = IDerivationState.STALE;
                else if (d.dependenciesState === IDerivationState.UP_TO_DATE)
                  observable.lowestObserverState = IDerivationState.UP_TO_DATE;

                consoleDDS(d, `[propagateChangeConfirmed][after][${i}]`);

                consoleLowest(
                  observable,
                  `[propagateChangeConfirmed][after][${i}]`
                );
              }
            }
            function propagateMaybeChanged(observable) {
              consoleLowest(observable, `[propagateMaybeChanged][before]`);
              if (
                observable.lowestObserverState !== IDerivationState.UP_TO_DATE
              )
                return;
              ...
              consoleLowest(observable, `[propagateMaybeChanged][after]`);
              ...
              while (i--) {
                ...
                consoleDDS(d, `[propagateMaybeChanged][before][${i}]`);
                ...
                consoleDDS(d, `[propagateMaybeChanged][after][${i}]`);
              }
            }
            var Reaction = (function() {
              ...
              Reaction.prototype.track = function(fn) {
                ...
                var result = mockup.trackDerivedFunction(this, fn, undefined);
                this._isRunning = false;
                this._isTrackPending = false;
                if (this.isDisposed) {
                  clearObserving(this);
            ...
            function onReactionError(handler) {
              ...
              return function() {
                var idx = globalState.globalReactionErrorHandlers.indexOf(
                  handler
                );
                if (idx >= 0)
            ...
            function asReference(value) {
              deprecated(
                'asReference is deprecated, use observable.ref instead'
              );
            ...
            function asFlat(value) {
              deprecated(
                'asFlat is deprecated, use observable.shallow instead'
              );
            ...
            var ObservableArray = (function(_super) {
              ...
              ObservableArray.prototype.toString = function() {
                this.$mobx.atom.reportObserved();
                return Array.prototype.toString.apply(
                  this.$mobx.values,
                  arguments
                );
              };
            ...
            [
              ...
              invariant(
                ...
                "Base function not defined on Array prototype: '" +
                  funcName +
                  "'"
              );
            ...
            function runLazyInitializers(instance) {
              ...
              if (instance.__mobxLazyInitializers) {
                ...
                instance.__mobxDidRunLazyInitializers &&
                  instance.__mobxLazyInitializers.forEach(function(
                    initializer
                  ) {
            ...
            function isES6Map(thing) {
              if (
                getGlobal().Map !== undefined &&
                thing instanceof getGlobal().Map
              )
                return true;
              return false;
            }
...
var globalState = mobx.extras.getGlobalState();
var depth = -1;
...
var startTrace = (fnName, args) => {
  depth++;
  console.trace(...startStr(depth, fnName, args));
};
var endTrace = (fnName, retval) => {
  console.trace(...endStr(depth, fnName, retval));
  depth--;
};

Tracing.traceFn = function(name) {
  return Tracing.before(name, function(fnName, args) {
    startTrace(fnName, args);
  }).after(name, function(fnName, retval) {
    endTrace(fnName, retval);
  });
  return Tracing;
};

Tracing.traceFn('mockup.shouldCompute')
  .traceFn('mockup.trackDerivedFunction')
  .traceFn('mobx.Reaction.prototype.track')
  .traceFn('mockup.reportObserved')
  .traceFn('mockup.bindDependencies');
